...
- **"Check for update" no longer shows "Up to date" before the check has actually finished** — `reg.update()` resolving only means the update-check request completed, not that no update was found; a new service worker installs asynchronously afterward. The button now shows "Checking for update…" until it's actually known whether an update was found (via the same `needRefresh` signal `UpdateBanner` uses), then resolves to either "Up to date" or hands off cleanly to the existing "Update available" banner — the two can no longer show at the same time.
